...
Plot_point<- function (Mass_flipper_data){
  Mass_flipper_data %>%
    ggplot(aes(x= body_mass_g,y = flipper_length_mm))+
    geom_point()+
    geom_smooth(method = "lm", color = "Red", linewidth = 0.8, fill = "pink")+
    labs (x= "Body Mass (mm)", y = "Flipper Length (mm)", title="Body mass vs flipper length in palmers Penguins", caption = "Pink Shaded Area indicates 95% CI")+
    theme_bw()
}
